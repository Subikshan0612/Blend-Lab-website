...
// Adapted for Blend Lab: reduced-motion renders one stable frame.
...
const useIsStaticRenderer = useReducedMotion
...
function __OriginkitBase_VolumetricText(props: VolumetricTextProps) {
    ...
    const drawMask = useCallback(() => {
        ...
        const baseSize = (() => {
            const fs = f.fontSize
            const scale = Math.max(0.7, Math.min(1.2, Math.max(w, 1) / REFERENCE_WIDTH))
        ...
        if (p.fitToWidth) {
            const fits = (s: number) => {
                const l = layout(s, availW)
                return l.widest <= availW && l.height <= availH
            }
        ...
        const { wrapped } = layout(size, p.fitToWidth ? availW : availW)
        ...
        for (let i = 0; i < wrapped.length; i++) {
            ...
            if (line !== undefined) {
                ctx.fillText(line, x, h / 2 - total / 2 + lh * (i + 0.5))
            }
        }
    ...
    const render = useCallback((now: number) => {
        ...
        const sc = parseColor(p.shadowColor, [0, 0, 0])
    ...
    useEffect(() => {
        drawMask()
    }, [text, font, fitToWidth, fitPadding, align, drawMask])

    useEffect(() => {
        if (isStatic) render(performance.now())
    }, [isStatic, textColor, backgroundColor, shadowColor, rainbow, render])
    ...
    useEffect(() => {
        ...
        const onLeave = () => {
            ...
            const { w, h } = sizeRef.current
            targetRef.current = {
                x: (propsRef.current.lightX / 100) * w,
                y: (1 - propsRef.current.lightY / 100) * h,
            }
        }
...
export default function VolumetricText(props: Partial<VolumetricTextProps>) {
  return <__OriginkitBase_VolumetricText {...(__originkitPresetProps as Record<string, unknown>)} {...props} />;
}